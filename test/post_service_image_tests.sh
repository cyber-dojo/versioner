...
from_outside_run_all_tests()
{
  ...
  docker run \
    ...
    ${image} \
      "/app/test/post_service_image_tests.sh"
}

from_inside_run_all_tests()
{
  cd "${ROOT_DIR}/test"
  TEST_FILES=(test_post_*.rb)
...
on_ci_run_post_service_image_tests()
{
  if ! on_CI; then
    echo 'not in CI Workflow so not running POST service image tests'
  else
    echo 'in CI Workflow so running POST service image tests'
    post_service_image_tests
  fi
}
